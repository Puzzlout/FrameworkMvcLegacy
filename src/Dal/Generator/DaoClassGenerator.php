...
class DaoClassGenerator {
  ...
  public function __construct($tableName) {
    $this->dir = "APP_ROOT_DIR" . "Library/Dal/Generator/output/";
    $this->className = ucfirst($tableName);
    $this->fileName = $this->className . ".php";
    $this->placeholders = $this->InitPlaceholders();
    $this->isFrameworkClass =
            (preg_match("`" . \WebDevJL\Framework\Enums\GenericAppKeys::PREFIX_FRAMEWORK_TABLE . ".*$`", $tableName) ?
                    TRUE :
                    FALSE);
  }
...
//    }
  ...
  private function AddPropertyPhpDoc($columnMetadata, $isSetter = TRUE) {
    $output =
            $this->_TAB2 . \WebDevJL\Framework\GeneratorEngine\CodeSnippets\PhpDocSnippets::OPENING .
            $this->_TAB2 . $this->_LF;
    $placeholders = array(
